...
//   const printTicketContent = (ticket, isOrganizerQR = false) => {
//     if (isOrganizerQR) {
//       return `
//         <html>
//           <head>
//             <title>Organizer QR Code</title>
//             <style>
//               @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap');
//               body {
//                 font-family: 'Poppins', sans-serif;
...
//               <h2 style="color: #333; margin: 0 0 15px 0;">Organizer QR Code</h2>
//               <div class="qr-code">
//                 <img src="data:image/png;base64,${ticket.ticket_qr}" alt="Ticket QR Code" />
//               </div>
//               <div class="info-label">Event</div>
//               <div class="info-value">${event?.title || 'Event'}</div>
//               <div class="info-label">Ticket Type</div>
//               <div class="info-value">${ticket.ticket_type_details.name}</div>
//               <div class="info-label">Ticket ID</div>
...
//       const printWindow = window.open('', '_blank');
//       printWindow.document.write(printTicketContent(ticket));
//       printWindow.document.close();
//       printWindow.print();
//     } catch (err) {
//       setError('Failed to print ticket. Please try again.');
...
//   const handlePrintOrganizerQR = async (ticketId) => {
//     try {
//       const ticket = await ticketApi.printTicket(ticketId);
      
//       const printWindow = window.open('', '_blank');
//       printWindow.document.write(printTicketContent(ticket, true));
//       printWindow.document.close();
//       printWindow.print();
//     } catch (err) {
//       setError('Failed to print organizer QR code. Please try again.');
//     }
//   };
...
function Booking() {
  ...
  const fetchEventAndTickets = async () => {
    try {
      ...
      let processedData = Array.isArray(ticketTypesData) ? ticketTypesData : [];

      // If no ticket types, try to create from eventData.tickets
      if (processedData.length === 0 && eventData.tickets) {
        const ticketTypesToCreate = Object.entries(eventData.tickets)
          .filter(([type, details]) => details !== null);
        ...
        if (ticketTypesToCreate.length > 0) {
          ...
          processedData = createdTypes.filter(Boolean);
        }
      }

      // Group by ticket name to avoid duplicates
      const groupedTickets = processedData.reduce((acc, ticket) => {
        const key = ticket.name.toLowerCase();
        if (!acc[key]) {
          acc[key] = {
            ...ticket,
